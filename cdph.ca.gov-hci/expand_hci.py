...
ep.doc.get_or_new_section('OtherResources', "Name Schema Space Time StartLine HeaderLines Encoding Description".split())
...
for i, r in enumerate(p.datafiles):
    ...
    ep.add_resource(r.url, schema=r.schema, name=name, description=description, space= space, time= time)
...
for r in list(ep.resources):
    if "Dictionary" in r.url:
        ...
        ep.doc['OtherResources'].add_term(r.term)
